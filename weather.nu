...
export def main [
    ...
    let format_temp = { |val|
        ...
        if $text {
            ...
            let g = if $v_int >= $units.hot_limit {
                { s: "0xffff00", e: "0xff0000" } # Yellow -> Red
            } else if $v_int <= $units.cold_limit {
                { s: "0xffffff", e: "0x00ffff" } # White -> Cyan
            } else {
                { s: "0x00ff00", e: "0xffff00" } # Green -> Yellow
            }
            $"($val)($units.temp_label)" | ansi gradient --fgstart $g.s --fgend $g.e
        }
    }
    ...
    if $astro {
        ...
        let m_phase = ($current_astro.moon_phase)
        let m_illum = ($current_astro.moon_illumination)
        let m_emoji = if $text { "" } else { (do $get_moon_icon $m_phase $m_illum) }
        ...
        let output = {
            ...
            "Moon Phase": $"($m_emoji) ($m_phase)",
            "Illumination": $"($m_illum)%"
        }
    ...
    if $hourly {
        ...
        let i_snow = if $text { "" } else if $emoji { "❄ " } else { " " }
        ...
        let hourly_table = ($today.hourly | each { |hour|
            ...
            # Icons
            let w_code = ($hour | get -o weatherCode | default "113")
            let w_desc = ($hour | get -o weatherDesc.0.value | default "Unknown")
            let w_emoji = if $text { "" } else { (do $get_weather_icon $w_code) }
            
            # Wind
            let wind_speed = ($hour | get -o $units.speed_key | default "0")
            let wind_k = ($hour | get -o windspeedKmph | default "0")
            let b_scale = (do $get_beaufort $wind_k)
            let b_icon = (do $get_beaufort_icon $b_scale)
            let wind_dir_str = ($hour | get -o winddir16Point | default "N")
            let wind_dir = (do $get_wind_dir_icon $wind_dir_str)
            
            let wind_display = $"($b_icon) ($wind_speed)($units.speed_label) ($wind_dir)"
            ...
            let precip_display = if ($chance_snow | into int) > 0 {
                $"($i_snow)($chance_snow)% / ($precip_val)($units.precip_label)"
            } else {
                $"($i_rain)($chance_rain)% / ($precip_val)($units.precip_label)"
            }

            let condition_str = if $text { $w_desc } else { $"($w_emoji) ($w_desc)" }

            {
                Time: $time_display,
                Condition: $condition_str,
                Temp: (do $format_temp $temp),
                Feels: (do $format_temp $feels),
                Precip: $precip_display,
                Wind: $wind_display,
                Humidity: $"($i_humid)(($hour | get -o humidity | default '0'))%"
            }
        })
    ...
    if $forecast {
        ...
        let forecast_table = ($data.weather | each { |day|
            ...
            let w_code = ($noon | get -o weatherCode | default "113")
            let w_desc = ($noon | get -o weatherDesc.0.value | default "Unknown")
            let w_emoji = if $text { "" } else { (do $get_weather_icon $w_code) }
            
            # Moon for forecast
            let m_phase = ($day.astronomy.0.moon_phase)
            let m_illum = ($day.astronomy.0.moon_illumination)
            let m_emoji = if $text { "" } else { (do $get_moon_icon $m_phase $m_illum) }
            ...
            
            {
                Date: $date,
                Condition: $condition_str,
                High: (do $format_temp $max_temp),
                Low: (do $format_temp $min_temp),
                Rain: $"($precip_val)($units.precip_label)",
                Wind: $wind_display,
                Moon: $"($m_emoji) ($m_phase)",
    ...
    let temp_val = ($current | get -o $units.temp_key | default "0" | into int)
    ...
    let i_wind = if $text { $"($b_icon) " } else if $emoji { $"💨 ($b_icon) " } else { $"($b_icon) " }
    let i_rain = if $text { "" } else if $emoji { "☔ " } else { " " }
    let i_vis = if $text { "" } else if $emoji { "👁 " } else { " " }
    let i_press = if $text { "" } else if $emoji { "⏲ " } else { " " }
    let i_cloud = if $text { "" } else if $emoji { "☁ " } else { " " }
    let i_humid = if $text { "" } else if $emoji { "💧 " } else { " " }
    let i_uv = if $text { "" } else if $emoji { "☀ " } else { " " }
    ...
    let wind = $"($i_wind)($wind_speed)($units.speed_label) ($wind_dir)"
    let precip = $"($i_rain)(($current | get -o $units.precip_key | default '0.0'))($units.precip_label)"
    let vis = $"($i_vis)(($current | get -o $units.vis_key | default '0'))($units.vis_label)"
    let pressure = $"($i_press)(($current | get -o $units.press_key | default '0'))($units.press_label)"
    ...
    let w_emoji = if $text { "" } else { (do $get_weather_icon $weather_code) }
    ...
    let m_emoji = if $text { "" } else { (do $get_moon_icon $moon_phase $moon_illum) }
    
    let m_display = if $text { "Moon: " } else { $"($m_emoji) " }
    ...
    let a_reset = if $text { "" } else { (ansi reset) }
    let a_grey = if $text { "" } else { (ansi grey) }
    let a_uv = if $text { "" } else { (ansi $uv_color) }

    let temp_display = (do $format_temp $temp_val)

    let condition_display = if $text { $"($weather_desc)($alert_icon)" } else { $"($w_emoji) ($weather_desc)($alert_icon)" }
    ...
    let output = {
        ...
        Clouds: $"($i_cloud)(($current | get -o cloudcover | default '0'))%",
        Rain: $precip,
        Humidity: $"($i_humid)(($current | get -o humidity | default '0'))%",
        Wind: $wind,
        Pressure: $pressure,
        Visibility: $vis,
        UV: $"($i_uv)($a_uv)($uv)($a_reset)",
        Astronomy: $"($sunrise_display)(($astro | get -o sunrise | default 'N/A')) | ($sunset_display)(($astro | get -o sunset | default 'N/A')) | ($m_display)(($astro | get -o moon_illumination | default '0'))%",
        Updated: $"($a_grey)($update_str) \(Local\) / ($utc_str) \(UTC\)($a_reset)"
    }
    ...
    if $oneline {
        let oneline_emoji = if $text { "" } else { $"($w_emoji) " }
